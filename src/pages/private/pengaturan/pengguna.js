...
function Pengguna() {
  ...
  const saveDisplayName = async (e) => {
    ...
    if (!displayName) {
      ...
      setSubmitting(false);
      enqueueSnackbar("Data berhasil diperbaharui", { variant: "success" });
    }
  };
      ...
      try {
        ...
        switch (e.code) {
          ...
          case "auth/requires-recent-login":
            emailError =
              "Silahkan logout, kemudian login kembali untuk memperbaharui email";
            break;
          default:
  ...
  const sendEmailVerifiaction = async (e) => {
    const actionCodeSettings = {
      url: `${window.location.origin}/login`,
    };

    setSubmitting(true);
    await user.sendEmailVerifiaction(actionCodeSettings);
    enqueueSnackbar(
      `Email verivikasi telah dikirim ke ${emailRef.current.value}`,
      { variant: "success" }
    );
    setSubmitting(false);
  };
    ...
    if (!password) {
      ...
      try {
        ...
        switch (e.code) {
          ...
          case "auth/requires-recent-login":
            errorPassword =
              "Silahkan logout, kemudian login kembali untuk memperbaharui password";
            break;
          default:
